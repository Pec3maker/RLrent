...
import ru.surfstudio.android.datalistlimitoffset.domain.datalist.*
import ru.surfstudio.android.easyadapter.pagination.PaginationState
import ru.rlrent.i_network.network.error.NoInternetException
import ru.rlrent.ui.placeholder.LoadStateType
...
object RequestMappers {
    ...
    object DataMappers {
        ...
        fun <T> pagination(): RequestDataMapper<DataList<T>, PaginationBundle<T>, PaginationBundle<T>> =
            { request, paginationBundle ->
                val currentDataList: DataList<T>? = paginationBundle?.list?.let {
                ...
                val mappedPaginationState = when {
                    request.isSuccess && !hasMoreData || request.isError -> PaginationState.COMPLETE
        ...
        fun <T> pagination(
            ...
            { request, paginationBundle ->
                val currentDataList = paginationBundle?.list?.let {
                    DataList(it, it.limit, it.offset, it.totalCount)
                }
                val newDataList = request.getDataOrNull()
                ...
                val mappedPaginationState = when {
                    ...
                    !request.isError && hasMoreData -> PaginationState.READY
                    request.isError && hasCurrentData -> PaginationState.ERROR
                    else -> null
                }
    ...
    object LoadingMappers {
        ...
        fun <T1, T2> default(
            isSwr: Boolean = false,
        ): RequestLoadingMapper<T1, T2> =
            { request, data ->
                val hasInternetConnection = request.getErrorOrNull() !is NoInternetException
                ...
                when {
                    ...
                    request.isError && !hasData && !hasInternetConnection -> LoadStateType.NoInternet
    ...
    object ErrorHandlers {
         ...
         *
         * **Ошибки [NonAuthorizedException], [SessionExpiredException] и [NoInternetException] -- обрабатываются всегда!**
         *
         * @return Была ли обработана ошибка? (Всегда -- true)
         * */
        fun <T> loadingBased(errorHandler: ErrorHandler): RequestErrorHandler<T> =
            { error, _, loading ->
                ...
                if (isNonAuthorized || isNoInternet ||
                    !isErrorStateOnDisplay
                ) error?.also(errorHandler::handleError)
                true
            }
         ...
         * ## Обработчик ошибок. Обработка в зависимости от текущего [LoadStateType].
         *
         * Если [LoadStateType] == [LoadStateType.None] то ошибки будут прокидываться в errorHandler
         *
         * **[SessionExpiredException] обрабатывается всегда!**
         * */
        fun <T> loadingBasedIfNone(errorHandler: ErrorHandler): RequestErrorHandler<T> =
            { error, _, loading ->
                if (loading is LoadStateType.None) {
                    error?.also(errorHandler::handleError)
                }
                true
            }
        ...
        /**
         * ## Стандартный обработчик ошибок, который переводит ошибку обновления экрана в ErrorUpdateScreen()
         *
         * Следует использовать при обновлении экрана с помощью птр
         *
         * **[SessionExpiredException] обрабатывается всегда!** обрабатываются всегда
         * */
        fun <T> loadingBasedIfUpdateScreenError(errorHandler: ErrorHandler): RequestErrorHandler<T> =
            { error, _, loading ->
                if (loading is LoadStateType.None) {
                    error?.also(errorHandler::handleError)
                }
         ...
         *
         * Обрабатывает только ошибки [NonAuthorizedException] и [SessionExpiredException], остальные -- игнорируются.
         *
         * @return Была ли обработана ошибка? (Всегда -- true)
         * */
